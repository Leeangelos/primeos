...
export async function GET(request: Request) {
  try {
    ...
    const [productsRes, costsRes, hillcrestRes] = await Promise.all([
      productsQuery,
      storeUuid
        ? supabase.from("menu_item_costs").select("item_name, size, category, cost_to_make, includes_disposables, notes").eq("store_id", storeUuid)
...
export async function POST(request: Request) {
  try {
    ...
    let storeUuid: string = storeIdParam;
    if (!uuidRe.test(storeIdParam)) {
      const { data: storeRow } = await supabase.from("stores").select("id").eq("slug", storeIdParam).maybeSingle();
      if (!storeRow?.id) return NextResponse.json({ error: "store not found" }, { status: 400 });
    ...
    const { error } = await supabase.from("menu_item_costs").upsert(
      ...
      { onConflict: "store_id,item_name,size,category" }
    );
    if (error) {
